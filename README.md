...
- **just** — менеджер задач для экспорта и тестов (`brew install just` на macOS, `apt install just` на Ubuntu).
...
3. Запустите тесты: `just tests` (использует `GODOT_BIN`, заданный выше).
4. Откройте `project.godot` и проиграйте `scenes/Main.tscn`, чтобы посмотреть текущую сцену.

## Ежедневные команды

`just` настроен в корневом `justfile`. Все рецепты требуют настроенного `GODOT_BIN`.

| Команда | Назначение |
| --- | --- |
| `just` | Последовательно вызывает `web-itch`, `web-local`, `mac-debug`. |
| `just web-itch` | Экспорт web-сборки для itch.io (`builds/current/web_itch/` + zip). |
...
`just tests` — рекомендованный путь, но можно дернуть скрипт напрямую:
