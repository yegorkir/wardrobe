...
Скрипт скачивает Godot 4.5 (stable), кладет в пользовательский каталог и печатает нужные `export`.
Чтобы и установить, и проставить переменные в текущей сессии:

```bash
eval "$(scripts/tools/setup_godot.sh)"
```

Полезные переменные для переопределения:

- `GODOT_VERSION` (по умолчанию `4.5-stable`)
- `GODOT_INSTALL_DIR` (по умолчанию `~/.local/share/godot/<version>`)
- `GODOT_DOWNLOAD_URL` (если нужен кастомный источник)
