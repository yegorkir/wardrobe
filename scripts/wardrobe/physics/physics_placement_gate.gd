...
func decide_overlap(metrics: Dictionary, would_push_out_of_bounds: bool) -> Dictionary:
	...
	if affected > max_affected:
		return _decision(Decision.REJECT, "reject_big_overlap_affected")
	if max_ratio > max_area_ratio:
		return _decision(Decision.REJECT, "reject_big_overlap_area")
	if max_push > max_push_per_item_px:
		return _decision(Decision.REJECT, "reject_big_overlap_push_item")
	if total_push > max_push_total_px:
		return _decision(Decision.REJECT, "reject_big_overlap_push_total")
	if would_push_out_of_bounds:
		return _decision(Decision.REJECT, "reject_big_overlap_bounds")
	if max_push < ignore_push_px:
		return _decision(Decision.ALLOW, "")
	return _decision(Decision.ALLOW_NUDGE, "")

func _decision(decision: int, reason: String) -> Dictionary:
	return {
		"decision": decision,
		"reason": reason,
	}
