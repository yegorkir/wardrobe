...
func evaluate(run_state: RunState) -> ShiftWinResultScript:
	if run_state == null:
		return ShiftWinResultScript.new(false, REASON_NO_TARGET)
	if run_state.shift_status == RunStateScript.SHIFT_STATUS_FAILED:
		return ShiftWinResultScript.new(false, REASON_SHIFT_FAILED)
	...
	if run_state.checkout_done < run_state.target_checkout:
		return ShiftWinResultScript.new(false, REASON_NO_TARGET)
	return ShiftWinResultScript.new(true, REASON_ALL_TARGETS_MET)
