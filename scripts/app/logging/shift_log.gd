...
func record(event_type: StringName, payload: Dictionary = {}) -> void:
	...
	if event_type == &"VAMPIRE_STAGE_COMPLETE":
		...
		var loss: float = float(payload.get(&"loss", 0.0))
		var sources: Array = payload.get(&"sources", [])
		print("VAMPIRE_STAGE_COMPLETE item=%s stage=%d loss=%.1f sources=%s" % [item_id, stage, loss, sources])
		...
		var zombie_loss: float = float(payload.get(&"loss", 0.0))
		var rate: float = float(payload.get(&"rate", 0.0))
		print("ZOMBIE_STAGE_COMPLETE item=%s stage=%d loss=%.1f rate=%.2f" % [zombie_item_id, zombie_stage, zombie_loss, rate])
