...
### 2025-12-22 — Отказ от legacy-взаимодействий на Node-уровне (Task Analysis)
- Цель: удалить legacy-подход взаимодействий (`interaction_engine_legacy.gd`, `interaction_target.gd`) при сохранении текущего геймплейного поведения `wardrobe_scene`.
- Предварительный поиск показал отсутствие прямых ссылок в `scripts/` и `scenes/`, требуется подтвердить отсутствующие зависимости в сценах/скриптах.
- После удаления нужен повторный анализ на предмет других legacy-участков.

### 2025-12-22 — Анализ сложности и бэклог упрощений
- Запрос: определить, что в коде слишком сложно и может быть упрощено, приоритизируя архитектурную чистоту.
- Провёл обзор ключевых модулей (`scripts/ui/wardrobe_scene.gd`, `scripts/ui/wardrobe_step3_setup.gd`, `scripts/app/desk/desk_service_point_system.gd`, `scripts/domain/interaction/interaction_engine.gd`) и подготовил приоритетный бэклог упрощений.
- Итоги и архитектурные предложения зафиксированы в `docs/notes/2025-12-22_architecture_simplification_backlog.md`.
...
### 2025-12-22 — План: удалить legacy Node-взаимодействия
1. Перепроверить отсутствие ссылок на `interaction_engine_legacy.gd` и `interaction_target.gd` в `scripts/` и `scenes/`.
2. Удалить `scripts/wardrobe/interaction_engine_legacy.gd` и `scripts/wardrobe/interaction_target.gd`.
3. Прогнать GdUnit4 unit-набор (`./addons/gdUnit4/runtest.sh -a ./tests/unit`) и убедиться, что `wardrobe_scene` опирается только на доменный pipeline.
4. Зафиксировать результаты и выполнить новый анализ на оставшиеся legacy-участки.
