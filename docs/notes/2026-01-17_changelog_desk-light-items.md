...
- Re-ran `GODOT_TEST_HOME="$PWD/.godot_test_home_persist" task tests` after the fix; suite exited 0 with an existing CA cert error line and a slow test-scan warning.
- Launched Godot with `"$GODOT_BIN" --path .` to validate runtime startup; log included an existing lambda-capture freed error during runtime activity.
