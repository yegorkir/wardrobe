...
2) Добавить в prefab:
   ...
   - `PatienceBarBg` + `PatienceBarFill` (ColorRect) ИЛИ один `TextureProgressBar`
3) ВАЖНО:
   - всем новым Control: `mouse_filter = IGNORE`
   - визуал клиента/бар не перекрывают предметы:
     - расположить выше/сбоку от зоны предмета
     - и/или выставить меньший `z_index`, чем у DeskSlot/ItemAnchor
4) Добавить маленький script на root prefab (если проще), который кеширует NodePath в `_ready()` и даёт API:
...
- тикать терпение только у клиентов, которые сейчас на столах:
  - для каждого desk_state: current_client_id
  - patience_by_client_id[id] = max(patience_by_client_id[id] - delta, 0)
  - если стало 0: FAIL → end_shift
- `_clients_ui.refresh()`
...
1) В месте, где WorkdeskScene применяет desk_events:
   - если event.type == <FOUND_EVENT_TYPE>: `_served_clients += 1`
2) WIN:
...
Если этого не происходит — это сигнал, что wiring RunManager/навигации сломан, и это отдельный баг, не лечить его рефактором Step 5.
Достаточно зафиксировать проблему и минимально починить routing, не трогая домен/Step 4 DnD.
