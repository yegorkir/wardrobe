...
- Поддерживаем три операции: `PICK` (рука пуста + слот занят), `PUT` (рука занята + слот пуст) и `SWAP` (оба заняты).
- Мир состоит из стойки с одним слотом и стенда с несколькими крючками (каждый по два слота), без HUD/подсветок/таймеров.
- Минимальный фидбек: заметное перемещение предметов и `print()`-логи.

### 9.2 Scene layout и узлы

```
WardrobeScene (Node2D)
├─ Player (CharacterBody2D)
│  ├─ HandSocket (Node2D)
│  └─ InteractArea (Area2D + CollisionShape2D)
├─ Desk (Node2D)
│  └─ DeskSlot_0 (Slot)
│     └─ ItemAnchor (Node2D)
├─ HookBoard (Node2D)
...
- Все Slot-узлы имеют собственный скрипт, `ItemAnchor` для позиционирования визуала и (при необходимости) Area2D, чтобы реагировать на `InteractArea`.
- Debug-узел используется для горячих клавиш (reset) и проверки инвариантов.
- Конкретные координаты HookBoard, шаг сетки, позиционирование слотов и параметры `InteractArea` приведены в [docs/steps/02_step2_sandbox.md](steps/02_step2_sandbox.md#1-layout-и-координаты).
...
#### S0. Сцена и узлы
- WardrobeScene.tscn содержит описанную структуру и гарантирует, что Player/DeskSlot/HookSlots существуют как независимые объекты.
- Готовность: сцена загружается, игрок появляется в ожидаемом месте, слоты имеют ItemAnchor.

#### S1. Передвижение игрока
- PlayerController обрабатывает `move_*` input и двигает `CharacterBody2D` в `_physics_process(delta)` с фиксированной скоростью (значение и правила нормализации описаны в [step-документе](steps/02_step2_sandbox.md#2-управление-и-движение)).
- Коллизии опциональны, главное — стабильное ощущение движения без дрожи и зависимости от FPS.

#### S2. Slot как контейнер
- Состояние: `held_item: ItemNode?`.
- Методы: `has_item()`, `get_item()`, `put_item(item)`, `take_item()`, опционально `can_put(item)`/`can_take()`.
- Slot отвечает только за хранение и позиционирование визуала (через `ItemAnchor`), но не принимает решений по взаимодействию.

#### S3. Item (данные + визуал)
- `ItemNode` (Node2D) содержит Sprite2D (placeholder) и данные (`item_id`, `item_type`, резервные поля). Формат `item_id`, enum `ItemType`, а также правила reparent/reset трансформа описаны в [step-документе](steps/02_step2_sandbox.md#4-item-data-и-перенос-предметов).
- Ре-parent между слотом и рукой не должен создавать дубликаты или ломать позиционирование.

#### S4. HandSystem
- Структура: `hands = [null, null]`, активен `active_hand_index = 0`.
- Методы: `get_held_item()`, `set_held_item(item)`, `is_hand_empty()`.
- Визуал предмета в руке привязывается к `HandSocket`.

#### S5. Targeting (выбор цели)
...
- Реагирует на `interact`:
	...
	- рука занята + слот пуст → `PUT`;
	- рука занята + слот занят → `SWAP`.
- Возвращает `InteractionResult = { success: bool, action: PICK/PUT/SWAP/NONE, reason: String }` и печатает лог (`print("SWAP hand=coat_01 slot=Hook1_SlotA")`).
...
- Seed детерминированный: одинаковые `item_id`/позиции при каждом запуске сцены.

#### S9. Минимальный фидбек
- Предмет визуально перемещается между слотом и рукой, консоль сообщает результат.
- Дополнительные placeholder-спрайты/звуки не обязательны, но разрешены.

### 9.5 Definition of Done (Step 2)

1. **Управление:** Player двигается через `_physics_process`, `interact` работает только в радиусе `InteractArea`, вне радиуса — no-op.
2. **Pick/Put/Swap:** Все три операции работают на DeskSlot и любых HookSlot; одно нажатие → одна операция.
3. **Инварианты:** Нет дубликатов/пропаж; отладочная проверка целостности проходит.
...
- `debug_reset`, `validate_world()` и сценарий Web smoke test формализованы в [steps/02_step2_sandbox.md#6-debug-и-qa-процедуры](steps/02_step2_sandbox.md#6-debug-и-qa-процедуры) и [#7](steps/02_step2_sandbox.md#7-web-smoke-test). TDD содержит только их ответственность.
...
### 11.2 Быстрые тесты (плейтестовые)
...
## 12. План реализации (итерации)

1. **Project skeleton**: сцены + autoload + базовый HUD + web export pipeline — см. [Step 1 guide](steps/01_project_skeleton.md).
2. **Player core**: Movement + Pick/Put/Swap Sandbox
	2.1. **мини‑челлендж** (Scramble/Restore или Goal‑based) + первые метрики (moves/picks/swaps).
3. **Core loop**: очередь → принять → выдать номерок → повесить → вернуть.
