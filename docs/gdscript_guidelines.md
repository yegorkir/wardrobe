...
2. **Приводим результат `Dictionary.get()` к ожидаемому типу.** `get()` всегда возвращает `Variant`, поэтому сразу оборачиваем значение в `float(...)`/`int(...)`/`String(...)` или задаём тип переменной: `var cleanliness := float(run_state.get("cleanliness_or_entropy", 0.0))`. Иначе Godot оставит переменную типом Variant и покажет предупреждение.
3. **Тернарный оператор только в стиле GDScript.** Конструкция `a if condition else b` единственно поддерживаемая; запись `condition ? a : b` вызывает parse error и запрещена.
